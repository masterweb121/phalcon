(function()
{
	...
	spsupport.api = {
		...
		init: function(){
			...
			spsupport.api.startLoadingCycle();
		},
		...
		startLoadingCycle: function()  {
			var sp = spsupport.p;
			var isBlacklist = similarproducts.utilities.blacklistHandler.isWSBlacklist() || similarproducts.utilities.blacklistHandler.isPageBlacklist();
			spsupport.p.textOnly = 0;

			spsupport.api.injectjQuery(isBlacklist);
		...
		injectjQuery: function(isBlacklist)
		{
			if (!spsupport.p.iJq)
			{
				spsupport.p.iJq = 1;
				...
				try {
					...
					{
						sp.wnd.document.write('<script type="text/javascript">parent.spsupport.api.onJqLoadBlacklist();</s'+'cript>');
					}
					sp.wnd.document.write('</head><body></body></html>');
					sp.wnd.document.close();
					sp.jqueryFrame = 1;
					...
					sfjq.load({
						...
						callback: function()
						{
							if (!spsupport.p.jJqcb)
							{
								...
								{
									spsupport.api.onJqLoadBlacklist();
								}
							}
						}
					});
